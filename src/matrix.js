 ...
 * @return {Boolean}
 */
const isNumeric = (n) => !isNaN(parseFloat(n)) && isFinite(n) //  函数用于检查其参数是否是无穷大

// 每 矩阵matrix 乘以 行 point
function multiplyMatrixAndPoint (matrix, point) {
  console.log(matrix, point, 'matrix, point');
  // c -> column
  // r -> row 列 (竖着的hang就是列)
...
function generateMatrix (matrices) { 
  ...
  let inputMatrix = matrices[0] // inputMatrix => 第一次变换

  // 对N次变换进行循环 两两进行计算
  for(let i = 1; i < matrices.length; i++) {
    console.log(inputMatrix, matrices[i], 'inputMatrix, matrices[i]');
...
const generateCSS = (...values) => { // values: arr[] => 每次变换的记录
...
// export api
export { scale, translate, rotate, generateCSS }

// 第一步 将scale, translate, rotate 导出 根据各自规则计算出相应矩阵
// 第二步 将多个矩阵按照规则相乘
// 第三步 数组join 拼接 matrix3D前缀